...
import type { Agent, NyxiCommands } from 'nyxi-for-vscode'
import { Agents, detect, resolveNyxiCommandStatement } from 'nyxi-for-vscode'
...
export async function run(command: NyxiCommands, uri: Uri, options: RunOptions = {}) {
   if (uri) {
      ...
      const commandStatement = resolveNyxiCommandStatement(
         command,
         agent,
